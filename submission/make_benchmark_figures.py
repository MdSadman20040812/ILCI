...
# IEEE submission settings: 300 dpi, vector-friendly
plt.rcParams.update({
    'font.family': 'serif',
    'font.size': 9,
    'axes.titlesize': 10,
...
fig, axes = plt.subplots(1, 3, figsize=(7.5, 2.4), dpi=300)
...
fig.suptitle('ILCI Benchmark: Training Loss Curves on ATmega328P', fontsize=10, fontweight='bold')
fig.tight_layout()
fig.savefig(out / 'fig_loss_curves.pdf', format='pdf')
fig.savefig(out / 'fig_loss_curves.png', dpi=300)
plt.close(fig)
...
for ax, task in zip(axes, tasks):
    ...
    ax.grid(True, alpha=0.25)
axes[1].legend(frameon=True, fontsize=8, loc='lower right')
fig.suptitle('ILCI Benchmark: Classification Accuracy on ATmega328P', fontsize=10, fontweight='bold')
fig.tight_layout()
fig.savefig(out / 'fig_accuracy_curves.pdf', format='pdf')
fig.savefig(out / 'fig_accuracy_curves.png', dpi=300)
plt.close(fig)
...
for i, init in enumerate(inits):
    ...
    bars = ax1.bar(x + (i - 1) * width, accs, width, label=init, color=colors[init], edgecolor='black', linewidth=0.4)
    for b in bars:
        h = b.get_height()
        if h == 0:
            ax1.text(b.get_x() + b.get_width() / 2, 0.02, 'NaN', ha='center', va='bottom', fontsize=7, rotation=90)
        else:
            ax1.text(b.get_x() + b.get_width() / 2, h + 0.01, f'{h:.2f}', ha='center', va='bottom', fontsize=7)
...
for i, init in enumerate(inits):
    ...
    bars = ax2.bar(x + (i - 1) * width, vals, width, label=init, color=colors[init], edgecolor='black', linewidth=0.4)
    for b in bars:
        h = b.get_height()
        ax2.text(b.get_x() + b.get_width() / 2, h + max(vals) * 0.02, str(int(h)), ha='center', va='bottom', fontsize=7)
...
fig.suptitle('ILCI Benchmark Summary: ATmega328P, 3 Runs', fontsize=10, fontweight='bold')
fig.tight_layout()
fig.savefig(out / 'fig_summary_bars.pdf', format='pdf')
fig.savefig(out / 'fig_summary_bars.png', dpi=300)
plt.close(fig)
...
bp = ax.boxplot(box_data, patch_artist=True, showmeans=True, meanline=True, widths=0.6)
...
fig.suptitle('ILCI Benchmark: Accuracy Distribution on ATmega328P', fontsize=10, fontweight='bold')
fig.tight_layout()
fig.savefig(out / 'fig_accuracy_boxplot.pdf', format='pdf')
fig.savefig(out / 'fig_accuracy_boxplot.png', dpi=300)
plt.close(fig)

print('High-resolution submission figures written to', out)
